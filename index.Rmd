...
**First**, make sure you've loaded the infer package
```{r}
library(infer)
```

**Second**, use specify to clarify the dependent and independent varaibles:
```{r}
bootstrap <- data_here |>
  specify(dependent_var ~ independent_var)
```
  
**Third**, now we can actually do the work to generate the bootstrap:
```{r}
bootstrap <- data_here |>
  specify(dependent_var ~ independent_var) |>
...
**Fourth**, now that we've done the work to actually generate the bootstrap, we can do differnet things with it.
